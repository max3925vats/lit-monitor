...
and an "All New Results" section.
...
def _format_paper_list(
    ...
    *,
    ...
    for p in papers:
        ...
        # Link the title itself to doi.org when a DOI exists (standard Markdown,
        # so it works in Obsidian too); otherwise render a plain bold title.
        if doi:
            title_md = f"[**{title}**](https://doi.org/{doi})"
        else:
            title_md = f"**{title}**"
        line = f"- {title_md} · {author_str} ({year}) · score: {score:.3f}"
