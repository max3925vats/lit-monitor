"""Unit tests for /api/discovery/{start,stop,status,controls} (F4.2)."""
...
@pytest.mark.unit
def test_discovery_status_idle(client):
...
@pytest.mark.unit
def test_discovery_start_refuses_when_already_running(client):
    """If slot.process is alive, POST /start returns 409."""
    fake = MagicMock()
    fake.returncode = None
    fake.pid = 999
    runtime = get_runtime()
    runtime.processes["discovery"].process = fake
    runtime.processes["discovery"].started_at = "2026-05-26T00:00:00+00:00"

    r = client.post("/api/discovery/start", data={"dry_run": "false"})
...
@pytest.mark.unit
def test_discovery_stop_when_idle_returns_false(client):
...
@pytest.mark.unit
def test_discovery_controls_renders_start_buttons_when_idle(client):
